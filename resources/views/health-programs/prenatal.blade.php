...
@section('content')
    <div class="page-content">
        ...
        <div
            ...
            <form method="GET" id="prenatalFilterForm" class="filters"
                style="flex: 1; display: flex; gap: 12px; align-items: center;">
                <input type="text" name="search" id="prenatalSearch" placeholder="Search patient name or ID"
                    class="search-input" style="flex: 1; min-width: 300px;">

                <select name="purok" id="purokFilter" class="filter-select">
                    <option value="">Filter by Purok</option>
                    <option value="Sto. Niño">Purok 1</option>
                    <option value="Poblacion">Purok 2</option>
                </select>

                <select name="lmp_status" id="lmpStatusFilter" class="filter-select">
                    <option value="">Filter by LMP status</option>
                    <option value="recent">Recent (≤ 4 weeks)</option>
                    <option value="follow-up">Follow-up needed</option>
                </select>

                <input type="date" name="date" id="prenatalDateFilter" class="filter-select" />

                <button type="button" id="clearPrenatalFilters" class="btn btn-secondary"
                    style="padding: 10px 15px; font-size: 14px;">
                    <i class="bi bi-x-circle"></i> Clear
                </button>

                <button type="button" class="btn btn-primary" id="openPrenatalForm"
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="javascript:void(0)" class="btn-action btn-view view-prenatal"
                                    data-id="{{ $record->id }}"
                                    data-record="{{ $record->record_no ?? 'PT-' . str_pad($record->id, 3, '0', STR_PAD_LEFT) }}">
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const toggleForm = (showForm) => {
                    ...
                    if (filterWrapper) filterWrapper.style.display = showForm ? 'none' : 'flex';
                ...
                const prenatalForm = document.getElementById('prenatalFilterForm');
                const prenatalSearch = document.getElementById('prenatalSearch');
                const purokFilter = document.getElementById('purokFilter');
                const lmpStatusFilter = document.getElementById('lmpStatusFilter');
                const prenatalDateFilter = document.getElementById('prenatalDateFilter');
                const clearPrenatalBtn = document.getElementById('clearPrenatalFilters');

                let prenatalSearchTimeout;

                // Auto-submit on search input with debounce
                prenatalSearch.addEventListener('input', function () {
                    clearTimeout(prenatalSearchTimeout);
                    prenatalSearchTimeout = setTimeout(() => {
                        prenatalForm.submit();
                    }, 500);
                });

                // Auto-submit on filter change
                purokFilter.addEventListener('change', () => prenatalForm.submit());
                lmpStatusFilter.addEventListener('change', () => prenatalForm.submit());
                prenatalDateFilter.addEventListener('change', () => prenatalForm.submit());

                // Clear all filters
                clearPrenatalBtn.addEventListener('click', function () {
                    prenatalSearch.value = '';
                    purokFilter.value = '';
                    lmpStatusFilter.value = '';
