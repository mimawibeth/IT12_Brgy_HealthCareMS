...
@section('page-title', 'Immunization Records')
...
@section('content')
    <div class="page-content">
        ...
        <div
            ...
            <form method="GET" id="nipFilterForm" class="filters"
                style="flex: 1; display: flex; gap: 12px; align-items: center;">
                <input type="text" name="search" id="newNipSearch" placeholder="Search child name or record ID"
                ...
                <input type="date" name="date" id="dateFilter" class="filter-select" />

                <button type="button" id="clearNipFilters" class="btn btn-secondary"
                    style="padding: 10px 15px; font-size: 14px;">
                    <i class="bi bi-x-circle"></i> Clear
                </button>

                <button type="button" class="btn btn-primary" id="openNewNipForm"
                            ...
                            <td class="actions">
                                ...
                                @if(auth()->user()->role === 'super_admin')
                                    <a href="javascript:void(0)" class="btn-action btn-delete"
                                        onclick="openDeleteModal({{ $item->id }}, '{{ $item->child_name }}')">
        ...
        <div class="modal" id="deleteModal" style="display:none;">
            <div class="modal-content" style="max-width: 500px;">
                <div class="modal-header" style="background: #dc2626; color: white;">
                    <h3 style="margin: 0;"><i class="bi bi-exclamation-triangle-fill"></i> Delete NIP Record</h3>
                    <span class="close-modal" onclick="closeDeleteModal()"
                        style="color: white; cursor: pointer; font-size: 28px;">&times;</span>
                </div>
                <div class="modal-body">
                    <p style="margin-bottom: 20px;">Are you sure you want to permanently delete the immunization record for
                    ...
                    <div
                        ...
                        <p style="margin: 0; color: #991b1b; font-size: 13px;">All immunization data including visits and
                            vaccine history will be permanently deleted.</p>
                    </div>
                    ...
                    <form method="POST" id="deleteForm" action="">
                        ...
                        <div class="form-actions" style="display: flex; gap: 10px; justify-content: flex-end;">
                            ...
                            <button type="submit" class="btn btn-primary" style="background: #dc2626;"
                                onclick="this.form.action='/health-programs/new-nip/' + document.getElementById('deleteRecordId').value">
    ...
    @push('scripts')
        <script>
            ...
            const nipForm = document.getElementById('nipFilterForm');
            const nipSearch = document.getElementById('newNipSearch');
            const ageGroupFilter = document.getElementById('ageGroupFilter');
            const statusFilter = document.getElementById('statusFilter');
            const dateFilter = document.getElementById('dateFilter');
            const clearNipBtn = document.getElementById('clearNipFilters');

            let searchTimeout;

            // Auto-submit on search input with debounce
            nipSearch.addEventListener('input', function () {
            ...
            ageGroupFilter.addEventListener('change', () => nipForm.submit());
            statusFilter.addEventListener('change', () => nipForm.submit());
            dateFilter.addEventListener('change', () => nipForm.submit());

            // Clear all filters
            clearNipBtn.addEventListener('click', function () {
                nipSearch.value = '';
